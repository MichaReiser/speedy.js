...
export class FunctionFactory {
     ...
     */
    getOrCreateInstanceMethod(resolvedFunction: ResolvedFunction, numberOfArguments: number, context: CodeGenerationContext, linkage = llvm.LinkageTypes.InternalLinkage) {
        assert(resolvedFunction.instanceMethod && resolvedFunction.classType, "Resolved function needs to be an instance method");

        return this.getOrCreateStaticOrInstanceFunction(resolvedFunction, numberOfArguments, context, linkage, resolvedFunction.classType);
    }

    private getOrCreateStaticOrInstanceFunction(resolvedFunction: ResolvedFunction, numberOfArguments: number, context: CodeGenerationContext, linkage: llvm.LinkageTypes, objectType?: ts.ObjectType) {
        ...
        if (!fn) {
            fn = this.createFunction(mangledName, resolvedFunction, numberOfArguments, context, linkage, objectType);
        }
    ...
    protected createFunction(mangledName: string, resolvedFunction: ResolvedFunction, numberOfArguments: number, context: CodeGenerationContext, linkage: llvm.LinkageTypes, objectType?: ts.ObjectType) {
        const llvmArgumentTypes = this.getLLVMArgumentTypes(resolvedFunction, numberOfArguments, context, objectType);
    ...
    private getLLVMArgumentTypes(resolvedFunction: ResolvedFunction, numberOfArguments: number, context: CodeGenerationContext, objectType?: ts.ObjectType) {
        const argumentTypes = objectType ? [toLLVMType(objectType, context) ] : [];
