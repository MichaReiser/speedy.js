    ...
    write() {
        for (const [sourceFile, context] of Array.from(this.codeGenerationContexts.entries())) {
            ...
            if (!context.module.empty) {
                const oFileName = sourceFile.fileName.replace(".ts", ".o");
                const bcFileName = oFileName.replace(".o", ".bc");
                const sFileName = oFileName.replace(".o", ".s");

                llvm.writeBitcodeToFile(context.module, oFileName);
